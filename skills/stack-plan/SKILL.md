...
3. **品質チェックポイントを省略しない** — 各Phase完了時のチェック項目は必須。省略すると品質が担保されない。
...
仕様書の要件をレビュアーが確認しやすいように、独立した小さな「スタックPR」単位に分割する。プロジェクトのアーキテクチャに応じて適切な分割方針を選択する。

例（レイヤードアーキテクチャの場合）:
1. **PR 1**: ドメイン層（エンティティ、バリューオブジェクト）
2. **PR 2**: データアクセス層（Repository インターフェース、実装）※親: PR 1
3. **PR 3**: サービス層（ビジネスロジック）※親: PR 2
4. **PR 4**: プレゼンテーション層（Controller、バリデーション）※親: PR 3
5. **PR 5**: 横断的関心事と統合テスト ※親: PR 4

各PRは「前のPRに依存して積み上げる（スタック）」前提で定義する。

**PRサイズの制約:**
- 各PRは **200〜400行以内** を目標とする
- 400行を超える場合はPRをさらに分割する
- 各PRが単独でレビュー・テスト可能であること
- マージは**ボトムアップ**（PR 1 → PR 2 → ...）で行い、**squash mergeは使用しない**（スタックが壊れるため）
...
### Step 5: 品質チェックポイントの挿入

各Phase完了時の確認項目を追加する。`references/` 配下にリファレンスが存在する場合はそれに基づいたチェックポイントを挿入する。基本チェックポイント:
- 仕様書との整合性
- ADRの決定事項への準拠
- テストカバレッジの確認
- コーディング規約の遵守
